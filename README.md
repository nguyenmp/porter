...
- Built-in skills are compiled into the binary instead of shipped as
  ...
  shipped guidance. `load_skill` serves their bodies from memory. Three ship
  ...
  the skill, so the two cannot drift), remote-file editing guidance
  ...
  and handoff summary guidance (`internal/handoff`: write the summary for the
  person who picks the work up next, in plain language, covering what the work
  was, where it stands, where it lives, the next step, and the traps).
...
- **Spooling tool output to files** — a `spool_output(call_id, path?)` tool (agent-served like recall, declared on every request) saves a tool result's full bytes to a file on the active execution provider, so the model can process output with `shell` (grep/jq/sed) and the file tools instead of paging it through context. The agent looks the result up in History and asks the provider to write it via a private `_porter_spool_write` tool, so the file lands on whatever provider is active (old turns, server-side MCP results, and handoffs all work — the source result just has to be in the conversation). Path is optional: the default is `.porter/out/<call_id>` under the working directory; the result reports the resolved absolute path. The bytes are written verbatim — a raw mirror of the committed result, no exit-code line stripped — so the file and History can never drift. The model view nudges toward it: truncated results (>16 KB) carry a spool clause in their truncation header next to the recall hint, and fully-shown results ≥1 KB (probably data, not prose) get a one-line footer hint — except outputs that are already on disk or prose (`read_with_line_numbers`, the edit tools, `load_skill`). *(Future: once a result is spooled, drop the head+tail copy from the model view; the file becomes the cheap copy and recall stays the fallback.)*
