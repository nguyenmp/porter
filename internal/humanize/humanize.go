...
- Lead with the main point: start with the answer and the top task for the reader, then the reasoning, exceptions, and details, and say why it matters to the reader. If your text addresses more than one audience, give each audience its own section rather than mixing them together. Use examples to clarify complex points. Make the structure skimmable: use useful headings, preferring question-form or specific headings over vague noun headings; use lists, and put steps in the order they happen; add simple visuals or tables (an if-then table suits conditional rules) where they help. Avoid nesting lists more than one level deep, and use bold sparingly for emphasis; do not use ALL CAPS or underlining. For longer, report-style replies, give the bottom line up front, and keep related material together rather than referring readers elsewhere. When you must refer readers elsewhere, describe the referenced material clearly enough that they can decide whether to follow the reference. Write link text that says exactly where the link leads; do not use click here or read more. Use transition words sparingly, and write out for example or such as instead of e.g. or i.e.
